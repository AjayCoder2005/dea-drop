...
const STYLES = `
  ...
  .pc-chart-open  { animation: chartReveal   0.35s ease-out forwards; overflow: hidden; }
`;
...
const ProductCard = ({ product: initialProduct }) => {
  ...
  // ── KEY FIX: chartKey forces full remount of PriceChart when toggled ──
  ...
  // ── Toggle chart — increment key to force fresh remount ──────────────────
  const handleToggleChart = () => {
    if (!showChart) {
      setChartKey(k => k + 1); // fresh fetch every open
      setShowChart(true);
    } else {
      setShowChart(false);
    }
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: "14px 16px", display: "flex", flexDirection: "column", gap: 10, flex: 1 }}>
        ...
        {/* Target input */}
        {showTargetInput && (
          <div style={{ display: "flex", gap: 8, alignItems: "center", animation: "slideDown .2s ease" }}>
            <input
              type="number"
            ...
            <button
              ...
              style={{ background: saving ? "#2a2a3a" : "#22c55e", color: "#fff", border: "none", borderRadius: 8, padding: "7px 14px", fontSize: 12, fontWeight: 600, cursor: saving ? "not-allowed" : "pointer", display: "flex", alignItems: "center", gap: 4, transition: "background .2s" }}
            >
        ...
        {/* Confirm delete */}
        {showConfirm && (
          <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 12px", borderRadius: 8, background: "rgba(244,63,94,0.08)", border: "1px solid rgba(244,63,94,0.25)", animation: "badgeIn .2s ease" }}>
      ...
      {/* ── Chart — key prop forces full remount on each open ────────────── */}
      {showChart && (
        <div className="pc-chart-open" style={{ padding: "0 16px 16px", borderTop: "1px solid #222230" }}>
          <PriceChart
            key={chartKey}
