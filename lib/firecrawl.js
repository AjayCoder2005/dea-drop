...
export async function scrapeProduct(url) {
  try {
    ...
    return {
      name: "Test Product",
      current_price: priceMatch ? priceMatch[1] : null,
  ...
  try {
    ...
    for (const product of products) {
      try {
        ...
        const newPrice = parseFloat(productData.current_price); // ✅ FIXED
        ...
        await supabase
          ...
          .update({
            ...
            name: productData.name || product.name,             // ✅ FIXED
            image_url: productData.image_url || product.image_url, // ✅ FIXED
